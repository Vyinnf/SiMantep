...
@push('styles')
    {{-- Jika admin-pendaftaran.css benar-benar dibutuhkan dan spesifik hanya untuk halaman ini --}}
...
@section('content')
    <div class="container"> {{-- Anda bisa tetap menggunakan container ini di dalam content --}}
        ...
        @if(session('success'))
            <div class="alert alert-success global-alert alert-dismissible fade show"> {{-- Sesuaikan dengan style alert di layout --}}
                    ...
                    <tbody>
                        @forelse($pendaftaran as $item)
                            <tr>
                                ...
                                <td>
                                    @if($item->status == 'pending')
                                        <form action="{{ route('admin.pendaftaran.verifikasi', $item->id) }}" method="POST" style="display:inline-block; margin-bottom: 0.25rem;">
                                            @csrf
                                            <button type="submit" class="btn btn-success btn-sm">
                                                <i class="mdi mdi-check"></i> Verifikasi
                                            </button>
                                        </form>
                                        <form action="{{ route('admin.pendaftaran.tolak', $item->id) }}" method="POST" style="display:inline-block;">
                                            @csrf
